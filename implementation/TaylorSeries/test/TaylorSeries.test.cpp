...
math::implementation::DataReferencingTaylorSeries<std::vector<double>::iterator>
...
int main( int argc, const char* argv[] ){
  LOG(INFO) << "";
  LOG(INFO) << "TaylorSeries Tests";
  LOG(INFO) << "=================================";
  int result = Catch::Session().run( argc, argv );
  LOG(INFO) << "TaylorSeries Tests Complete!";
  return result;
}

SCENARIO("The ctor will throw for invalid coefficient ranges",
         "[math], [TaylorSeries], [ctor]"){
  GIVEN("a zero length range"){
    WHEN("passed to the ctor"){
      THEN("the ctor will throw"){
        ...
        REQUIRE_THROWS(
          math::implementation::DataReferencingTaylorSeries
          < std::vector<double>::iterator >
          (coefficients.begin(), coefficients.begin(), false));
      }
    }
  }
    ...
    WHEN("passed to the ctor"){
      THEN("the ctor will throw"){
        ...
        REQUIRE_THROWS(
          math::implementation::DataReferencingTaylorSeries
          < std::vector<double>::iterator >
          (coefficients.begin(), coefficients.begin() - 1, false));
      }
    }
  }
}
