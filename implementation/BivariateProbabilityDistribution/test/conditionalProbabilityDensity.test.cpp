...
SCENARIO(
  ...
  "[math], [BivariateProbabilityDistribution], [conditionalProbabilityDensity]"){
  GIVEN( "valid conditions" ){
    WHEN( "the condition falls on a boundary" ){
      THEN( "the return value will match the reference" ){
        ...
        for( auto event : events ){
          REQUIRE( pdf0(event) == Approx( bvpd->conditionalProbabilityDensity
                                          ( event, condition ) ) );
        }
        condition = conditions[1];
        for( auto event : events ){
          REQUIRE( pdf1(event) == Approx( bvpd->conditionalProbabilityDensity
                                          ( event, condition ) ) );
        }
        condition = conditions[2];
        for( auto event : events ){
          REQUIRE( pdf2(event) == Approx( bvpd->conditionalProbabilityDensity
                                          ( event, condition ) ) );
        }
      }
    }
    ...
    WHEN( "the conditions are not on a boundary" ){
      ...
      for( auto event : events ){
        ...
        REQUIRE( reference == Approx( bvpd->conditionalProbabilityDensity
                                      ( event, condition) ) );
      }
    ...
    GIVEN( "valid events" ){
      THEN( "exceptions will be thrown" ){
        for ( double c : conditions ){
          REQUIRE_THROWS( bvpd->conditionalProbabilityDensity(0.0, c) );
        }
      }
    }
  } // GIVEN---invalid conditions
